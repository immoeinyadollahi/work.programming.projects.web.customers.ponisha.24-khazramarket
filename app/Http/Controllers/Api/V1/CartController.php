...
class CartController extends Controller
{
    ...
    public function store(StoreCartRequest $request)
    {
        ...
        return $this->respondSuccess($cart_product ? 'با موفقیت در سبد خرید بروزرسانی شد.' : 'با موفقیت به سبد خرید اضافه شد.');
    }

    public function destroy(Request $request, $price)
    {
        $cart = $this->getCart($request);
        if (!$cart->prices()->where("prices.id", $price)->exists()) abort(404);
    ...
    private function getCart(Request $request)
    {
        ...
        if ($request->user("sanctum")) {
            $cart = $request->user("sanctum")->getCart();
        } else {
