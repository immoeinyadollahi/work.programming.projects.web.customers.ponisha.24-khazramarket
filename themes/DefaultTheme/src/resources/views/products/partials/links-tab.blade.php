        ...
        <div class="container-fluid">
            @foreach ($product->prices()->whereHas('file')->orderBy('prices.ordering')->get() as $price)
                <div class="row files mb-2">
                    ...
                    <div class="col-md-2 col-5">
                        <p class="mb-0">{{ trans('front::messages.currency.prefix') }}{{ number_format($price->discountPrice()) }} {{ trans('front::messages.currency.suffix') }}</p>

                        @if ($price->discount)
                            <del class="text-danger">{{ trans('front::messages.currency.prefix') }}{{ number_format($price->tomanPrice()) }} {{ trans('front::messages.currency.suffix') }}</del>
                        @endif
                    </div>
