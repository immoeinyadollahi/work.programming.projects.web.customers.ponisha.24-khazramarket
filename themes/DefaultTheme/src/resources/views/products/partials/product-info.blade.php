...
<div class="col-lg-8 col-md-6 pb-5 product-info-block">
    <div class="product-info dt-sl">
        <div class="product-title">
            ...
            @can('products.update')
                <a class="btn btn-warning btn-sm admin-edit-url" target="_blank" href="{{ route('admin.products.edit', ['product' => $product]) }}"><i class="mdi mdi-pencil"></i> {{ trans('front::messages.products.edit') }}</a>
            @endcan
        </div>
        ...
        <div class="row pt-4">
            ...
            <div class="col-md-6">
                ...
                @if ($product->isPhysical() && $product->getPrices->count())
                    ...
                    @php
                        $selected_price = $product->getPriceWithAttributes($attributes_id) ?? $product->getPrices()->first();
                    @endphp
        ...
        @if($product->isPhysical() && $product->addableToCart())
            ...
            <div class="section-title text-sm-title title-wide no-after-title-wide mb-0 dt-sl">
                <h2>{{ trans('front::messages.products.price') }}<span class="price">{{ trans('front::messages.currency.prefix') }}{{ number_format($selected_price->discountPrice()) }} {{ trans('front::messages.currency.suffix') }} </span>
                    @if ($selected_price->discount)
                        <del class="text-danger">{{ trans('front::messages.currency.prefix') }}{{ number_format($selected_price->tomanPrice()) }} {{ trans('front::messages.currency.suffix') }}</del>
                    @endif
                </h2>
            </div>
