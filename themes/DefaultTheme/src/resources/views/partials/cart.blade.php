                    ...
                    <a href="{{ route('front.products.show', ['product' => $product]) }}" class="dropdown-item">
                        ...
                        <div class="mr-3 cart-title">
                            {{ $product->title }}
                                ...
                                <div class="pt-1">{{ trans('front::messages.currency.prefix') }}{{ number_format($cart_product_price->discountPrice() * $product->pivot->quantity) }} {{ trans('front::messages.currency.suffix') }}</div>
                                @if($cart_product_price->discount)
                                    <del class="text-danger">{{ trans('front::messages.currency.prefix') }}{{ number_format($cart_product_price->tomanPrice() * $product->pivot->quantity) }} {{ trans('front::messages.currency.suffix') }}</del>
                                @endif
